...
OUTPUT_ROOT = os.path.join(BASE_DIR, "output_seep_con_clustering_11ms")
...
CLUSTERING_MODEL_PATH  = os.path.join(BASE_DATA_PATH, "clustering_models", "km")
CLUSTERING_SCALER_PATH = os.path.join(BASE_DATA_PATH, "clustering_models", "km-scaler")
CLUSTER_INPUT_FEATURES = ["Ux", "Ut", "Ur"]
N_CLUSTERS = 7
...
CLUSTER_OHE_FEATURES = [f"cluster_{c}" for c in range(N_CLUSTERS)]
...
def add_cluster_features(features_df: pd.DataFrame, km, km_scaler) -> pd.DataFrame:
    """
    Apply the pre-trained KMeans to assign cluster one-hot columns.
    The scaler and model expect ['Ux', 'Ut', 'Ur'] in that order.
    """
    cluster_input = features_df[CLUSTER_INPUT_FEATURES].to_numpy()
    cluster_input_scaled = km_scaler.transform(cluster_input)
    labels = km.predict(cluster_input_scaled)
    print(f"[INFO] Cluster label distribution: { {c: int(np.sum(labels == c)) for c in range(N_CLUSTERS)} }")
    for c in range(N_CLUSTERS):
        features_df[f"cluster_{c}"] = (labels == c).astype(np.float32)
    return features_df
...
def main():
    print("=" * 72)
    print("Neural Network Training (single output: delta_nut, with clustering inputs)")
    print("=" * 72)
    ...
    print(f"[INFO] Clustering: KMeans n_clusters={N_CLUSTERS}, input features={CLUSTER_INPUT_FEATURES}")
    ...
    print("\n[2/8] Loading clustering models...")
    with open(CLUSTERING_MODEL_PATH, "rb") as f:
        km = pickle.load(f)
    with open(CLUSTERING_SCALER_PATH, "rb") as f:
        km_scaler = pickle.load(f)
    print(f"  KMeans loaded: {km.n_clusters} clusters")
    print(f"  Clustering scaler loaded: {type(km_scaler).__name__}")

    # --- 3. Build features ---
    print("\n[3/8] Building features...")
    features_df, nut_rans, nut_les = build_features(mesh_rans)
    features_df = add_cluster_features(features_df, km, km_scaler)
    ...
    # --- 5. Preprocessing ---
    print("\n[5/8] Preprocessing features and targets...")
    if SKIP_TRAINING:
        ...
        print(f"  Input dimension: {input_dim}  (= {len(NUMERIC_FEATURES)} numeric + {N_CLUSTERS} cluster OHE)")
        ...
        preprocessor = ColumnTransformer(
            [
                ...
                ("cluster", "passthrough", CLUSTER_OHE_FEATURES),
            ],
            remainder="drop",
        )
    ...
    if SKIP_TRAINING:
        print("\n[6-7/8] Skipped (model loaded from disk).")
        model = keras.models.load_model(MODEL_PATH)
        print(f"  Model loaded from: {MODEL_PATH}")

    # --- 8. Evaluation and output ---
    print("\n[8/8] Evaluating model and writing VTK...")
