\section{Results}
\label{sec:results}
Finally the results are presented. Must be noted that the ML results are referred to the finest grid, which is the one used for training. The ML-based RANS model results are referred to the coarser grid instead to stress the improvement of the ML-based RANS model with respect to the standard RANS model not only in terms of accuracy but also in terms of computational cost.
...
The network predicts the standardised correction $\Delta \nu_t=\nu_{t,eq}-\nu_{t,\mathrm{RANS}}$ and the final eddy viscosity is reconstructed as $\nu_{t,ML}=\nu_{t,\mathrm{RANS}}+\Delta \nu_{t,ML}$. In the reported run, early stopping is triggered at epoch 201 with the best checkpoint at epoch 187, where $validation\ loss=2.96\cdot10^{-3}$ and $validation\ MAE=1.99\cdot10^{-2}$ in normalized target space. On the training set the reconstructed physical field reaches $R^2=0.9935$, $RMSE=0.2546$ and $MAE=0.1789$, while the validation subset gives $R^2=0.9934$, $RMSE=0.2588$ and $MAE=0.1831$.
...
To highlight the importance of clustering in the methodology, the ML-based RANS model was compared against a variant trained without clustering, using the same network architecture and hyperparameters without all clustering-related components. The best validation loss for the unclustered model is obtained at epoch 313, with $validation\ loss \approx 1.16\cdot10^{-2}$ and $validation\ MAE = 1.96\cdot10^{-2}$ in the normalised target space. On the full dataset, the reconstructed physical field reaches $R^2 = 0.9854$, $MSE = 1.3556\cdot10^{-1}$ and $MAE = 0.1757$. Although the MSE and MAE are marginally lower than in the clustered case, the lower $R^2$ indicates that clustering allows the model to better capture the variance of the data and thus to reconstruct the physical field more faithfully. The training and validation loss curves together with the scatter plots of the predicted and true $\nu_t$ and $\Delta \nu_t$ values on the test set are shown in Fig.~\ref{fig:training_curves_no_clustering}.
...
In terms of computational cost, the ML-based RANS model on the $\sim 15.9$ million cells grid is about $2.5$ times more expensive than the standard LES, which could be regarded as a significant drawback. However, the real efficiency of the ML-based RANS model emerges only on the coarser grids.
...
\begin{figure}
    ...
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth, trim=80 120 80 150, clip=true]{results/img/ml_results/error_fields/rans_ml_5m_vs_les/error_U.png}
        \caption{}
    ...
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth, trim=80 120 80 150, clip=true]{results/img/ml_results/error_fields/rans_vs_les/error_U.png}
        \caption{}
